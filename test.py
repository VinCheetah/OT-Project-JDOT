...
X_train, y_train = next(iter(DataLoader(train_dataset_source, batch_size=len(train_dataset_source))))
X_test, y_test = next(iter(DataLoader(test_dataset_source, batch_size=len(test_dataset_source))))
...
print(f"Accuracy on source domain: {correct / len(y_test) * 100:.2f}%")

y_pred = model.predict(X_target)
correct = (y_pred == y_target).sum().item()
print(f"Accuracy on target domain: {correct / len(y_target) * 100:.2f}%")
